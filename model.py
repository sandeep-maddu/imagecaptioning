...
class Caption_Generator():
    ...
    def __init__(self, dim_image, dim_embed, dim_hidden, batch_size, n_lstm_steps, n_words, bias_init_vector=None):
		...
		# tf.add_to_collection('vars', embed_word_b)
		# tf.add_to_collection('vars', Wemb)
		# tf.add_to_collection('vars', bemb)
		# tf.add_to_collection('vars', embed_word_W)
		# tf.add_to_collection('vars', encode_img_W)
		# tf.add_to_collection('vars', encode_img_b)
    ...
    def build_generator(self, maxlen):
        ...
        with tf.variable_scope("RNN"):
            ...
            #print('Wemb inside build generator =',(self.Wemb))
...
annotation_path = os.path.join(data_path, 'captions.txt')
################################################################
script_path = os.path.dirname(__file__)

model_path = os.path.join(script_path, model_path)
vgg_path = os.path.join(script_path, vgg_path)
data_path = os.path.join(script_path, data_path)
feat_path = os.path.join(script_path, feat_path)
annotation_path = os.path.join(script_path, annotation_path)
...
def test(test_feat='./guitar_player.npy', model_path='./models/model-999', maxlen=30): # Naive greedy search

    ixtoword = np.load('data/ixtoword.npy').tolist()
    n_words = len(ixtoword)

    feat = [np.load(test_feat)]
	#feat = [np.load(open(r'./guitar_player.npy', 'rb'))]
    sess = tf.InteractiveSession()
    caption_generator = Caption_Generator(
           dim_image=dim_image,
           dim_hidden=dim_hidden,
...
def test_tf(test_image_path='./test1.jpg', model_path1='./models/model-999', maxlen=30):
    ...
    #fc7 = sess.run(graph.get_tensor_by_name("fc7/fc7:0"), feed_dict={images:image_val})
    print('fc7=',fc7)
    print('fc7 sum=',np.sum(fc7))
    print('fc7 shape=',fc7.shape)
    print('non-zero elements in fc7=',np.count_nonzero(fc7))
    ...
    saver = tf.train.Saver()
    #saver=tf.train.import_meta_graph('./models1.meta')
    #tf.reset_default_graph()
    #saver.restore(sess, model_path)
    saver.restore(sess, './models/model-999')
    #saver.restore(sess, tf.train.latest_checkpoint('./'))
    
	
    #print('Wemb before=',sess.run(caption_generator.Wemb))
	
    #sess.run(tf.global_variables_initializer())
	
    #print('Wemb after=',sess.run(caption_generator.Wemb))

    #generated_word_index= sess.run(generated_words, feed_dict={fc7_tf:fc7})
    generated_word_index= sess.run(generated_words, feed_dict={fc7_tf:fc7})
    print('generated_word_index1=',generated_word_index)
    generated_word_index = np.hstack(generated_word_index)
    print('generated_word_index2=',generated_word_index)
    ...
    #print('Wemb=',sess.run(caption_generator.Wemb))


def read_image_new(image_obj, target_height=224, target_width=224, as_float=True):
    ...
    img =  cv2.resize(resized_image, (target_height, target_width))
    if img.shape[2] == 4:
        img = img[:,:,:3]
...
def test_tf1(test_image_path='./test1.jpg', model_path='./models/model-9999', maxlen=30, image_obj=None):
    tf.reset_default_graph()
    ...
    ixtoword_path = os.path.join(script_path, './data/ixtoword.npy')
    ixtoword = np.load(ixtoword_path).tolist()

    n_words = len(ixtoword)

    image_val = read_image_new(image_obj)
...
if __name__ == '__main__':
    ...
    while True:
        # Python 3
        # image = Image.open(input('Enter location: '))
        # Python 2
        image = Image.open(raw_input('Enter location: '))
        print(test_tf1(image_obj=image))
